...
# Get port - support Railway PORT env variable, command line arg, or default 8000
...
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:var(--bg);color:var(--text);line-height:1.6;min-height:100vh;padding-bottom:60px}}
...
def render_page(title, content, nav_active="", is_premium=False, username=None):
    ...
    if username:
        if is_premium:
            ...
            badge = '<span style="font-size:.65em;color:var(--dim)">普通用户</span>'
            multibet_link = '<a href="/multibet">多串一</a>'
        logout = f'<span style="color:var(--dim);font-size:.85em">{username}</span> {badge} <a href="/logout">退出登录</a>'
        login_link = logout
    else:
    ...
    _search_js = '<script>' + ''.join([
        '(function(){',
        'var SD=[{n:"墨西哥",f:"🇲🇽",g:"A",u:"/match/Mexico/South%20Africa?stage=group"},{n:"韩国",f:"🇰🇷",g:"A",u:"#group-A"},{n:"南非",f:"🇿🇦",g:"A",u:"#group-A"},{n:"捷克",f:"🇨🇿",g:"A",u:"#group-A"},',
...
def get_match_info(home_name, away_name):
    """Get match date, time, venue, and round information."""
    for m in ALL_MATCHES:
        if (m.get("home") == home_name and m.get("away") == away_name) or \
           (m.get("home") == away_name and m.get("away") == home_name):
            return {
                "date": m.get("date", ""),
                "time": m.get("time", ""),
                "venue": m.get("venue", ""),
                "city": m.get("city", ""),
                "round": m.get("round", m.get("group", "")),
                "status": m.get("status", ""),
                "home_score": m.get("home_score"),
                "away_score": m.get("away_score"),
            }
    return {"date": "", "time": "", "venue": "", "city": "", "round": "", "status": ""}
...
def verify_password(password: str, stored_hash: str, salt: str = None) -> bool:
    if len(stored_hash) == 16:
        old_hash = hashlib.sha256(password.encode()).hexdigest()[:16]
...
def needs_migration(user_data: dict) -> bool:
    return "salt" not in user_data

def check_lockout(username: str) -> tuple:
    now = time.time()
    if username in LOCKOUT_USERS:
        lockout_until = LOCKOUT_USERS[username]
        if now < lockout_until:
            remaining = int(lockout_until - now)
            return True, f"登录失败次数过多，请在{remaining}秒后再试"
        else:
            del LOCKOUT_USERS[username]
...
def record_login_failure(username: str):
    ...
    if username not in LOGIN_FAILS:
        LOGIN_FAILS[username] = []
    LOGIN_FAILS[username].append(now)
    LOGIN_FAILS[username] = [t for t in LOGIN_FAILS[username] if now - t < 300]
    if len(LOGIN_FAILS[username]) >= 5:
        LOCKOUT_USERS[username] = now + 900

def generate_csrf_token(session_token: str) -> str:
    csrf = uuid.uuid4().hex
    CSRF_TOKENS[csrf] = {"token": session_token, "expires": time.time() + 3600}
    return csrf

def validate_csrf_token(csrf: str, session_token: str) -> bool:
    if csrf not in CSRF_TOKENS:
        return False
    data = CSRF_TOKENS[csrf]
    if data["token"] != session_token:
        return False
    if time.time() > data["expires"]:
        del CSRF_TOKENS[csrf]
        return False
    del CSRF_TOKENS[csrf]
    return True
...
def validate_password(password: str) -> tuple:
    ...
    common_passwords = ['password', '12345678', 'admin123', 'qwerty', '123456', '000000', '888888']
...
def handle_home(token):
    ...
    for d in sorted_dates:
        ...
        date_selector += f'<div class="cal-date-card{active_cls}" data-date="{d}" onclick="showDate(\'{d}\')">'
    ...
    function showDate(dateStr) {
        ...
        var activeCard = document.querySelector('.cal-date-card[data-date="' + dateStr + '"]');
        if (activeCard) {
    ...
    <section class="hero">
        ...
        <div class="hero-cta">
            {"" if premium else '<a href="/pricing" class="btn btn-primary">购买全车套餐 - ¥2999</a>'}
        </div>
    </section>
...
def handle_model(token):
    ...
    <section class="section">
        <h2 class="section-title" style="border-left-color:#10B981">L4 心理层</h2>
        <p style="color:var(--dim);margin-bottom:16px">分析球队大赛经验、历史交锋心理优势等主观因素。</p>
        <div class="card-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
            <div class="card">
                <h3 style="font-size:1rem;margin-bottom:8px"><span style="color:#10B981">🏆</span> 大赛经验系数</h3>
                <p style="color:var(--dim);font-size:.85em">基于球队世界杯/欧洲杯参赛次数和最佳成绩，计算大赛心理适应度。</p>
            </div>
            <div class="card">
                <h3 style="font-size:1rem;margin-bottom:8px"><span style="color:#F59E0B">⚖️</span> 历史交锋心理</h3>
                <p style="color:var(--dim);font-size:.85em">近10年对阵记录结合胜负权重，量化球队间的心理优势/劣势。</p>
            </div>
        </div>
    </section>
...
def handle_match_detail(token, home_name, away_name):
    ...
    match_info = get_match_info(home_name, away_name)
    ...
    # Match date/time info - Beijing Time (UTC+8)
    match_datetime = ""
    if match_info.get("date") and match_info.get("time"):
        match_datetime = f"{match_info['date']} {match_info['time']}（北京时间）"
    elif match_info.get("date"):
        match_datetime = match_info["date"]
    ...
    username = get_session_user(token) or ""
    venue_display = f"{match_info.get('venue', '')} - {match_info.get('city', '')}".strip(" -")
    content = f"""
    <div class="match-detail" style="max-width:700px;margin:0 auto">
        <div style="text-align:center;padding:30px 0">
            ...
            <div style="margin-top:16px;color:#f97316;font-weight:600;font-size:.85em">
                <span>📅 {match_datetime}</span>
                {f'<br><span style="color:var(--dim);font-weight:400">📍 {venue_display}</span>' if venue_display else ''}
            </div>
        </div>
...
def handle_select_match(token, query_string=""):
    ...
    current_plan = plan_info.get(plan, plan_info["all"])
    ...
    if plan == "all":
        ...
        filtered_matches = [pred for pred in predictions if hasattr(pred, 'group') and pred.group and pred.group != "KO"][:48]  # First 48 group matches
        ...
        filtered_matches = [pred for pred in predictions if hasattr(pred, 'group') and pred.group and pred.group != "KO" and (pred.home_team in hot_teams or pred.away_team in hot_teams)][:20]
        match_type_label = "小组赛热门场次"
    elif plan == "knockout":
        # Knockout stage matches
        filtered_matches = [pred for pred in predictions if hasattr(pred, 'group') and pred.group == "KO"]
        match_type_label = "淘汰赛"
    elif plan == "semifinal":
        # Semi-final matches
        filtered_matches = [pred for pred in predictions if hasattr(pred, 'group') and pred.group == "KO" and "SF" in str(pred)]
        match_type_label = "半决赛"
    elif plan == "final":
        # Final match
        filtered_matches = [pred for pred in predictions if hasattr(pred, 'group') and pred.group == "KO" and "FIN" in str(pred)]
        match_type_label = "决赛"
    else:
        filtered_matches = predictions
    ...
    for i, match in enumerate(filtered_matches):
        home_cn = tcn(match.home_team)
        away_cn = tcn(match.away_team)
        match_id = f"{match.home_team}_{match.away_team}"

        # Determine if this is a hot match
        is_hot = any(team in ["Brazil", "Argentina", "Germany", "France", "Spain", "England", "Portugal", "Italy"] for team in [match.home_team, match.away_team])
        ...
        matches_html += f"""
        <label class="match-select-item" style="display:flex;align-items:center;gap:12px;padding:12px;border-bottom:1px solid var(--border);cursor:pointer">
            <input type="checkbox" class="match-checkbox" id="{match_id}" name="matches" value="{match_id}" style="width:18px;height:18px;cursor:pointer">
            <div style="flex:1">
                ...
                <div style="font-size:.78em;color:var(--dim)">
                    {match.most_likely_score[0]}:{match.most_likely_score[1]} · 置信度{match.confidence*100:.0f}%
                </div>
            </div>
    ...
    <div class="page-header">
        <h1>选择比赛</h1>
        <p>{current_plan['name']} · {match_type_label}</p>
    </div>
    ...
    <div class="card" style="max-width:800px;margin:0 auto">
        <div style="padding:16px;border-bottom:1px solid var(--border)">
            ...
            <div style="display:flex;justify-content:space-between;font-size:.9em">
                ...
                <span style="font-weight:700;color:var(--accent2)">¥{current_plan['price']}</span>
            </div>
            <p style="font-size:.82em;color:var(--dim);margin-top:8px">{current_plan['desc']}</p>
        </div>

        <form method="get" action="/payment" id="selectForm">
            ...
            <div style="padding:16px;border-top:1px solid var(--border);background:rgba(255,255,255,.02)">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                    ...
                    <span style="font-size:1.1em;font-weight:700;color:var(--warn)">合计: ¥<span id="totalPrice">{current_plan['price']}</span></span>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {{
        const unitPrice = {current_plan['price']};
        const planType = '{plan}';
        ...
        function updateSummary() {{
            ...
            let total = (planType === 'all') ? unitPrice : (selectedCount * unitPrice);
...
def handle_login(token, error_msg=""):
    ...
    if premium and username:
        content = f"""
        <div class="login-box">
            <h2 style="text-align:center;margin-bottom:24px">已登录</h2>
            <div style="text-align:center;padding:20px;background:rgba(45,138,62,.08);border-radius:12px;margin-bottom:16px">
                <div style="font-size:1.2em;margin-bottom:8px">👋 欢迎回来</div>
                <div style="font-size:1.5em;font-weight:700;color:var(--accent)">{username}</div>
                <div style="font-size:.85em;color:var(--dim);margin-top:8px">
                    {'<span class="badge-pro">PRO 会员</span>' if premium else ''}
                </div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                <a href="/" class="btn btn-primary" style="text-align:center">返回首页</a>
                <a href="/logout" class="btn btn-outline" style="text-align:center">退出登录</a>
            </div>
        </div>"""
        return render_page("登录 - 波胆教父", content, "", premium, username)
    ...
    <div class="login-box">
        ...
        <form method="post" action="/login">
            ...
            <div class="form-group">
                ...
                <p style="font-size:.75em;color:var(--dim);margin-top:4px">至少8位，需包含字母和数字（Railway部署版本 v2）</p>
            </div>
            <button type="submit" name="action" value="login" class="btn btn-primary" style="width:100%">登录</button>
        </form>
        <hr style="border-color:var(--border);margin:20px 0">
        <form method="post" action="/login" id="registerForm" onsubmit="return validateRegister()">
            <div class="form-group">
                <label>新用户名</label>
                <input type="text" name="username" id="regUsername" class="form-input" placeholder="设置用户名" required oninput="checkUsername()">
                <p id="usernameHint" style="font-size:.75em;color:var(--dim);margin-top:4px">3-20位，支持中文、字母、数字、下划线</p>
            </div>
            <div class="form-group">
                <label>设置密码</label>
                <input type="password" name="password" id="regPassword" class="form-input" placeholder="设置密码" required oninput="checkPasswordStrength()">
                <div id="passwordStrength" style="margin-top:4px;font-size:.75em">
                    <div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden;margin-bottom:4px">
                        <div id="strengthBar" style="height:100%;width:0;background:var(--dim);border-radius:3px;transition:all .3s"></div>
                    </div>
                    <span id="strengthText" style="color:var(--dim)">输入密码查看强度</span>
                </div>
                <p style="font-size:.75em;color:var(--dim);margin-top:4px">至少8位，需包含字母和数字（Railway部署版本 v2）</p>
            </div>
            <div class="form-group">
                <label>确认密码</label>
                <input type="password" id="regConfirmPassword" class="form-input" placeholder="再次输入密码" required oninput="checkPasswordMatch()">
                <p id="confirmHint" style="font-size:.75em;color:var(--dim);margin-top:4px">请再次输入密码以确认</p>
            </div>
    ...
    function checkUsername() {{
        ...
        if (username.length === 0) {{
            hint.textContent = '3-20位，支持中文、字母、数字、下划线';
            hint.style.color = 'var(--dim)';
        }} else if (username.length < 3) {{
            hint.textContent = '用户名至少需要3个字符';
    ...
    function checkPasswordStrength() {{
        ...
        if (strength <= 2) {{
            ...
            text.textContent = '密码强度：中等 可进一步增强';
...
def handle_warmup(token):
    ...
    <div class="wu-empty"><div class="wu-empty-icon">📡</div>热身赛数据正在从API同步中，请稍后刷新...</div>"""
        premium, _ptype, _reason = check_premium(token)
        username = get_session_user(token)
        return render_page("热身赛 - 波胆教父", content, "warmup", premium, username)
    ...
    for date_str, day_matches in matches_by_date.items():
        ...
        for m in day_matches:
            ...
            time_html = f'<div class="wu-time">📍 北京时间 {m["time"]}</div>'
        ...
        <div class="wu-card">
            <div class="wu-card-head">
                <span class="wu-card-date">📍 北京时间 {m['time']}</span>
    ...
    <div class="wu-cta-row">
        ...
        <a href="/pricing" class="btn btn-outline">购买正赛预测</a>
    </div>
    <script>
...
def handle_admin(token):
    ...
    if not premium or ptype != "all":
        return render_page("管理后台 - 波胆教父", '<div style="text-align:center;padding:60px 20px"><div style="font-size:4em;margin-bottom:16px">🚫</div><h1>403 禁止访问</h1><p style="color:var(--dim)">此区域仅限管理员访问。</p></div>', "", premium, username)
...
def handle_multibet(token):
    """Multi-bet combo predictions (score + 1X2). Enhanced with better UI and preview."""
    premium, ptype, _reason = check_premium(token)
    username = get_session_user(token) or ""
    predictions = get_all_predictions()
    ...
    def blur_value(val, show_val):
        if show_val:
            return str(val)
        return '<span style="filter:blur(4px)">•••</span>'
    ...
    for i, pick in enumerate(top_score_picks, 1):
        ...
        <tr style="border-bottom:1px solid var(--border)">
            ...
            <td style="padding:12px 8px;font-weight:600;vertical-align:middle">{pick['match']}</td>
            <td style="padding:12px 8px;text-align:center;color:var(--accent2);font-weight:700;vertical-align:middle">{blur_value(pick['pick'], premium)}</td>
            <td style="padding:12px 8px;text-align:center;vertical-align:middle">{blur_value(f"{pick['prob']:.1f}%", premium)}</td>
            <td style="padding:12px 8px;text-align:center;vertical-align:middle">{blur_value(f"{pick['confidence']:.0f}%", premium)}</td>
        </tr>"""
    ...
    combo_score_prob = 1.0
    for pick in top_score_picks:
        combo_score_prob *= (pick["prob"] / 100)
    combo_result_prob = 1.0
    for pick in top_result_picks:
        combo_result_prob *= (pick["prob"] / 100)

    lock_overlay = ""
    if not premium:
        lock_overlay = """
        <div style="position:absolute;inset:0;background:rgba(10,10,10,0.75);backdrop-filter:blur(3px);display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:10;border-radius:12px">
            <div style="font-size:3em;margin-bottom:16px">🔒</div>
            <h3 style="margin-bottom:12px;color:#fff">升级全车套餐解锁</h3>
            <p style="color:rgba(255,255,255,0.7);font-size:.85em;margin-bottom:20px;text-align:center">购买全车套餐后，可查看完整多串一预测数据</p>
            <a href="/payment?plan=all" class="btn btn-primary" style="font-weight:700">购买全车套餐 ¥2999</a>
        </div>"""

    content = f"""
    <div class="page-header">
        <h1 style="display:flex;align-items:center;gap:12px">🎯 组合投注预测</h1>
        <p>AI精选多串一方案 · 全车套餐专属</p>
    </div>

    <section class="section">
        <div class="card-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px">
            <div class="card" style="background:linear-gradient(135deg,rgba(59,130,246,0.08),rgba(14,165,233,0.08));border-color:rgba(59,130,246,0.2)">
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
                    <span style="font-size:1.8em">📊</span>
                    <div>
                        <div style="font-weight:700">比分多串一</div>
                        <div style="font-size:.75em;color:var(--dim)">精选5场高置信比分</div>
                    </div>
                </div>
                <div style="font-size:2em;font-weight:800;color:var(--accent)">{blur_value(f"{combo_score_prob*100:.2f}%", premium)}</div>
                <div style="font-size:.75em;color:var(--dim);margin-top:4px">组合理论命中率</div>
            </div>
            <div class="card" style="background:linear-gradient(135deg,rgba(16,185,129,0.08),rgba(52,211,153,0.08));border-color:rgba(16,185,129,0.2)">
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
                    <span style="font-size:1.8em">⚖️</span>
                    <div>
                        <div style="font-weight:700">胜平负多串一</div>
                        <div style="font-size:.75em;color:var(--dim)">精选5场高胜率预测</div>
                    </div>
                </div>
                <div style="font-size:2em;font-weight:800;color:#10B981">{blur_value(f"{combo_result_prob*100:.2f}%", premium)}</div>
                <div style="font-size:.75em;color:var(--dim);margin-top:4px">组合理论命中率</div>
            </div>
            <div class="card" style="background:linear-gradient(135deg,rgba(196,160,53,0.08),rgba(245,158,11,0.08));border-color:rgba(196,160,53,0.2)">
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
                    <span style="font-size:1.8em">🤖</span>
                    <div>
                        <div style="font-weight:700">AI模型驱动</div>
                        <div style="font-size:.75em;color:var(--dim)">泊松+贝叶斯双模型</div>
                    </div>
                </div>
                <div style="font-size:2em;font-weight:800;color:#C4A035">24小时</div>
                <div style="font-size:.75em;color:var(--dim);margin-top:4px">赛前24小时更新</div>
            </div>
        </div>
    </section>
                ...
                <div style="overflow-x:auto">
                    <table style="width:100%;border-collapse:collapse;font-size:.9em">
                        <thead><tr style="color:var(--accent2);font-size:.8em;text-align:left">
                            ...
                            <th style="padding:12px 8px">对阵</th>
                            <th style="padding:12px 8px;text-align:center">推荐比分</th>
                            <th style="padding:12px 8px;text-align:center">比分概率</th>
                            <th style="padding:12px 8px;text-align:center">模型置信度</th>
                        </tr></thead>
                ...
                <div style="overflow-x:auto">
                    <table style="width:100%;border-collapse:collapse;font-size:.9em">
                        <thead><tr style="color:var(--accent2);font-size:.8em;text-align:left">
                            <th style="padding:12px 8px">#</th>
                            <th style="padding:12px 8px">对阵</th>
                            <th style="padding:12px 8px;text-align:center">推荐结果</th>
                            <th style="padding:12px 8px;text-align:center">结果概率</th>
                            <th style="padding:12px 8px;text-align:center">模型置信度</th>
                        </tr></thead>
    ...
    <section class="section">
        <div class="card" style="background:rgba(196,160,53,.04);border-color:rgba(196,160,53,.15)">
            <h3 style="font-size:1em;margin-bottom:12px;display:flex;align-items:center;gap:8px">💡 组合建议与投注策略</h3>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px">
                <div style="padding:12px;background:rgba(255,255,255,0.02);border-radius:8px">
                    <div style="font-weight:700;margin-bottom:6px;color:#C4A035">场次选择</div>
                    <p style="color:var(--dim);font-size:.85em;line-height:1.6">建议将多串一场次控制在3-5场。场次越多组合概率越低，高收益伴随高风险。</p>
                </div>
                <div style="padding:12px;background:rgba(255,255,255,0.02);border-radius:8px">
                    <div style="font-weight:700;margin-bottom:6px;color:#C4A035">概率门槛</div>
                    <p style="color:var(--dim);font-size:.85em;line-height:1.6">单场比分概率低于15%、胜平负概率低于55%的推荐请谨慎选择组合。</p>
                </div>
                <div style="padding:12px;background:rgba(255,255,255,0.02);border-radius:8px">
                    <div style="font-weight:700;margin-bottom:6px;color:#C4A035">动态更新</div>
                    <p style="color:var(--dim);font-size:.85em;line-height:1.6">每场比赛开赛前24小时，模型会根据最新伤病报告、盘口变化进行最终修正。</p>
                </div>
                <div style="padding:12px;background:rgba(255,255,255,0.02);border-radius:8px">
                    <div style="font-weight:700;margin-bottom:6px;color:#C4A035">风险控制</div>
                    <p style="color:var(--dim);font-size:.85em;line-height:1.6">建议组合投注金额占总预算的10%以下，切勿将所有资金投入单一组合。</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="section">
        <div class="card" style="background:rgba(239,68,68,0.04);border-color:rgba(239,68,68,0.15)">
            <h3 style="font-size:1em;margin-bottom:12px;color:#EF4444">⚠️ 风险提示与免责声明</h3>
            <div style="color:var(--dim);font-size:.85em;line-height:1.8">
                <p style="margin-bottom:8px">1. 多串一预测仅供参考，足球比赛存在不可预测的随机性（伤病、红牌、点球、天气等）。</p>
                <p style="margin-bottom:8px">2. 历史模型表现不代表未来收益，任何预测都无法保证100%准确。</p>
                <p style="margin-bottom:8px">3. 组合投注风险较高，请理性购彩，切勿过度投注，更不要借贷投注。</p>
                <p>4. 波胆教父不对任何投注决策负责，请根据自己的判断做出决定。</p>
            </div>
        </div>
    </section>
    """
...
class RequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def send_redirect(self, location, cookie_token=None):
        ...
        if cookie_token:
            self.send_header("Set-Cookie", f"session={cookie_token}; Path=/; Max-Age=2592000; HttpOnly; SameSite=Lax")
        self.end_headers()
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/home":
            ...
            html = handle_select_match(token, parsed.query)
            self.send_html(html)
        elif path == "/reset-data":
            # Reset all data (admin only)
            username = get_session_user(token)
            premium, ptype, reason = check_premium(token)
            if premium and ptype == "all":
...
"""
            ...
            svg_icon = b'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="45" fill="#1E40AF"/><circle cx="50" cy="50" r="40" fill="#1E3A8A" stroke="#60A5FA" stroke-width="2"/><circle cx="50" cy="50" r="5" fill="#FCD34D"/><path d="M50 10 L50 90" stroke="#60A5FA" stroke-width="1"/><path d="M10 50 L90 50" stroke="#60A5FA" stroke-width="1"/></svg>'
            self.send_response(200)
    ...
    def do_POST(self):
        ...
        if path == "/login":
            ...
            if action == "register":
                ...
                users[username] = {"password": pw_hash, "salt": salt, "premium": False, "premium_until": 0, "created": time.time()}
                save_json(USERS_FILE, users)
                ...
                if is_locked:
                    html = handle_login(token, error_msg=lock_msg)
                    self.send_html(html)
                    return
                # 验证用户名格式
                valid_u, err_u = validate_username(username)
                ...
                if not user:
                    ...
                    self.send_html(html)
                    return
                is_valid = verify_password(password, user.get("password", ""), user.get("salt"))
                if is_valid:
                    if needs_migration(user):
                        new_pw_hash, new_salt = hash_password(password)
                        users[username]["password"] = new_pw_hash
...
def main():
    ...
    if "admin" not in users:
        temp_pw = uuid.uuid4().hex[:20]
        users["admin"] = {"password": hashlib.sha256(temp_pw.encode()).hexdigest(),
                         "premium": True, "purchase_type": "all", "purchased_matches": [],
                         "premium_until": time.time() + 86400 * 365, "created": time.time()}
        save_json(USERS_FILE, users)
        print(f"  [SECURITY] Admin temp password: {temp_pw}")
        print("  [SECURITY] CHANGE ADMIN PASSWORD IMMEDIATELY!")
